...
In our project linear blend skinning technique was used for animating human body mesh.
...
In this project we have to deal with various file formats. \emph{C3D} file format stores motion data from motion capture system and \emph{BVH} stores hierarchical skeleton structure and its rotations/translations adopted from C3D data. \emph{BVH} data was used in our program to define skeleton with its joint structure and animate it, while as \emph{OBJ} files were used for storing meshes of different human body parts, that has to be loaded on to the skeleton.
