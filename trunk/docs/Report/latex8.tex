...
While implementing and testing our linear blend skinning algorithm on human model knee with only simple deformations, that only made joint rotation on one of the 3D axis, we were getting really promising results. It can be seen on figure \ref{knee_1axis}.
...
Though after introducing full human body animation and taking data from BVH motion data file, we got joint rotations on all 3 axis, what caused totally different final results for our linear blend skinning algorithm. Results can be seen on figure \label{knee_3axis}.
...
Other solution that could give positive results, is cutting out smaller bits of model.

%-------------------------------------------------------------------------
\Section{Conclusion}

This project showed how to animate human body model using data from motion capture system. 
As our animating program was a part of this project, this paper was only finished, when we had the whole program code running, we are now able to provide all the necessary steps needed. 
All the mathematical formulas were provided in a paper, needed for animating characters in a 3D world. Furthermore, we provided our own interpretation of the linear blend skinning technique, that deals with model skin deformations. 
